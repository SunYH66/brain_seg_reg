...
if __name__ == '__main__':
    ...
    if arg.platform == 'local':
        ...
        cfg.data_root = '/public_bme/home/liujm/Project/InfantSyn/data' #'/public_bme/home/sunyh/data/infant_brain_seg_reg'
        cfg.batch_size = 3
        cfg.num_workers = 3
    ...
    for i, data in enumerate(dataset.test_loader):
        ...
        with torch.no_grad(): # no gradients to accelerate
            ...
            model.fix_seg = to_one_hot(model.seg_fix)
            model.mov_seg = to_one_hot(model.seg_mov_1)
            model.mov_ori = model.img_mov_1
        ...
        #     fix_seg = model.fix_seg[0:cfg.batch_size, ...].cpu().numpy().squeeze()  # (1, 1, H, W, D) -> (H, W, D)
        #
