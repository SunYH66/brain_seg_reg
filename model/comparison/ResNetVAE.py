...
class GreenBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=32, norm="group"):
        ...
        if norm == "batch":
            ...
            raise NotImplementedError('Norm name [%s] is not recognized' % norm)
        ...
        self.layer_2 = nn.Sequential(
            nn.Conv3d(in_channels=in_channels, out_channels=in_channels, kernel_size=(3, 3, 3), stride=(1,1,1), padding=1),
            norm_2,
            nn.ReLU())
...
class VAE(nn.Module):
    def __init__(self, in_channels=256, in_dim=(10, 10, 10), out_dim=(2, 64, 64, 64)):
        ...
        relu_1 = nn.ReLU()
        conv_1 = nn.Conv3d(in_channels=in_channels, out_channels=self.encoder_channels, stride=(2,2,2), kernel_size=(3, 3, 3),
                           padding=1)
...
class ResNetVAE(nn.Module):
    def __init__(self, in_channels=1, classes=4, max_conv_channels=256, dim=(64, 64, 64)):
