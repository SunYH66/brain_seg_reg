...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.add_module('conv1', nn.Conv3d(num_input_features, bn_size *
                                           growth_rate, kernel_size=(1,1,1), stride=(1,1,1), bias=False)),
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.add_module('pool', nn.Conv3d(num_output_features, num_output_features, kernel_size=(2,2,2), stride=(2,2,2)))
...
class SkipDenseNet(nn.Module):
    ...
    def __init__(self, in_channels=1, classes=4, growth_rate=16, block_config=(4, 4, 4, 4), num_init_features=32, drop_rate=0.1,
                 bn_size=4):
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv3d(in_channels, num_init_features, kernel_size=(3,3,3), stride=(1,1,1), padding=1, bias=False)),
            ('norm0', nn.BatchNorm3d(num_init_features)),
            ('relu0', nn.ReLU(inplace=True)),
            ('conv1', nn.Conv3d(num_init_features, num_init_features, kernel_size=(3,3,3), stride=(1,1,1), padding=1, bias=False)),
            ('norm1', nn.BatchNorm3d(num_init_features)),
            ('relu1', nn.ReLU(inplace=True)),
            ('conv2', nn.Conv3d(num_init_features, num_init_features, kernel_size=(3,3,3), stride=(1,1,1), padding=1, bias=False)),
        ]))
        self.features_bn = nn.Sequential(OrderedDict([
            ('norm2', nn.BatchNorm3d(num_init_features)),
            ('relu2', nn.ReLU(inplace=True)),
        ]))
        self.conv_pool_first = nn.Conv3d(num_init_features, num_init_features, kernel_size=(2,2,2), stride=(2,2,2), padding=0,
                                         bias=False)
        ...
        for i, num_layers in enumerate(block_config):
            ...
            up_block = nn.ConvTranspose3d(num_features, classes, kernel_size=2 ** (i + 1) + 2,
                                          stride=2 ** (i + 1),
                                          padding=(1,1,1), groups=classes, bias=False)
        ...
        self.conv_class = nn.Conv3d(classes * 4 + num_init_features, classes, kernel_size=(1,1,1), padding=0)
        self.relu_last = nn.Softmax()
        # ----------------------------------------------------------
    ...
    def forward(self, x):
        ...
        print(first_three_features.max())
    ...
    def test(self,device='cpu'):
        ...
        assert ideal_out.shape == out[0].shape
