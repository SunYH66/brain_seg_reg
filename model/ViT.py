...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if conv_stem_configs is not None:
            ...
            seq_proj.add_module(
                "conv_last", nn.Conv3d(in_channels=prev_channels, out_channels=hidden_dim, kernel_size=1)
            )
        ...
        seq_length = (image_size[0] // patch_size) * (image_size[1] // patch_size) * (image_size[2] // patch_size)
        ...
        if isinstance(self.conv_proj, nn.Conv3d):
            # Init the patchify stem
            fan_in = self.conv_proj.in_channels * self.conv_proj.kernel_size[0] * self.conv_proj.kernel_size[1]
            nn.init.trunc_normal_(self.conv_proj.weight, std=math.sqrt(1 / fan_in))
            if self.conv_proj.bias is not None:
                nn.init.zeros_(self.conv_proj.bias)
        elif self.conv_proj.conv_last is not None and isinstance(self.conv_proj.conv_last, nn.Conv3d):
...
def _vision_transformer(
    ...
    **kwargs: Any,
    ...
    image_size = kwargs.pop("image_size", cfg.crop_size)
